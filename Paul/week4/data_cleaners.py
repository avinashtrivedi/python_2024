...
class JHUCoreData:
    ...
    def get_us_case_data(self):
        ...
        return pd.read_csv("../../assets/assignment4/time_series_covid19_confirmed_US.csv")

    def get_us_death_data(self):
        #keep it safe instead of going to website
        return pd.read_csv("../../assets/assignment4/time_series_covid19_deaths_US.csv")

    def get_global_case_data(self):
        #keep it safe instead of going to website
        return pd.read_csv("../../assets/assignment4/time_series_covid19_confirmed_global.csv")

    def get_global_death_data(self):
        #keep it safe instead of going to website
        return pd.read_csv("../../assets/assignment4/time_series_covid19_deaths_global.csv")
    
    def get_global_recovery_data(self):
        #keep it safe instead of going to website
        return pd.read_csv("../../assets/assignment4/time_series_covid19_recovered_global.csv")

class CUBData(JHUCoreData):
    ## can use FB data but not necessary
    ## for now, run with core features
    def __init__(self):
        super().__init__()
    ...
    def clean_JHU_cases(self):
        ...
        while current_date > first_date:
            ...
            jh_covid_df[previous_col] = \
                np.where(jh_covid_df[previous_col] > jh_covid_df[current_col], 
        ...
        smoothed.iloc[:, 4:] = jh_covid_df.iloc[:, 4:].rolling(7, min_periods=1, axis=1).mean()

        return jh_covid_df, smoothed # daily covid cases across US (original, smoothed)
    
    def combine_data(self, forecast_date, contiguous_counties, covid_df, covid_df_non_smooth):
        ...
        temp.drop(columns=dates_case_str, inplace=True) # drop both columns that had the smoothened data

        output = temp.merge(jh_df_non_smooth, on='FIPS', how='left') # add the two columns that have the actual case count
        output['DELTA_CASE_JH'] = output[dates_case_str[0]] - output[dates_case_str[1]] # actual case count for the previous week

        output.drop(columns=dates_case_str, inplace=True) # drop the two columns that have the actual case count
        # output has all columns from counties, current-date, DELTA_INC_RATE, DELTA_CASE_JH
        return output

    def get_data(self, current_date):
        
        counties_df = pd.read_csv('../../assets/assignment4/counties.csv')
        ...
        initial_date = date(int(INITIAL_DATE.split("-")[0]),
        ...
        while forecast_date >= initial_date:
            df_week = self.combine_data(forecast_date,
                                   counties_df,
                                   covid_df_contiguous,
...
class covidData():
    ...
    def __init__(self, current_date, init_df, temporal_lag=TEMPORAL_LAG, forecast_horizon=FORECAST_HORIZON):
    ...
    def load_dataframes(self, print_info=False):
        ...
        for col in na_cols:
            timeseries_df[col] = timeseries_df.groupby(["current_date","GEOID"])[col].transform(lambda x: x.fillna(x.mean()))

         # columns: 'GEOID', 'NAME', 'STATE', 'current_date', 'LOG_DELTA_INC_RATE', 'DELTA_CASE_JH'
        target_cols = [0,3,4,5]
        self.all_targets = timeseries_df.iloc[:,target_cols] # 'GEOID', 'current_date', 'LOG_DELTA_INC_RATE', 'DELTA_CASE_JH'
        if print_info:
            print("All TS shape", timeseries_df.shape)

        cols = [0,1,2,3,4]
        timeseries_df = timeseries_df.iloc[:,cols] # 'GEOID', 'NAME', 'STATE', 'current_date', 'LOG_DELTA_INC_RATE'
    ...
    def create_train_test_datasets(self, print_info=False):
        ...
        for i,id in enumerate(geoid_list):
            county_ts = self.all_ts_data[self.all_ts_data["GEOID"]==id].reset_index(drop=True)
            ...
            current_index = county_ts.index[county_ts["current_date"]==current_date][0]
            ...
            county_test_info = self.all_targets[self.all_targets["GEOID"]==id].values[:,0] # GEOID
            ...
            while y_index + temporal_lag + forecast_horizon <= current_index:
                ts_instance = county_ts[y_index:y_index+temporal_lag+1,:] # 0,1,2,..,8,9
                X_train_ts.append(ts_instance)
                y_train.append(county_ts[y_index+temporal_lag+forecast_horizon]) # 10 (when forecast_horizon == 1)
                if print_info:
                    if i==0:
                        print("Train X:", ts_instance)
                        print("Train Target: ", county_ts[y_index+temporal_lag+forecast_horizon])

                y_index += 1
            
            X_test_ts.append(county_ts[(current_index-temporal_lag):(current_index+1),:])

            # append y_test
            y_test.append(county_ts[-1,:]) # last row has the LOG_DELTA_INC_RATE for the target week (smoothened data)
            y_delta_cases.append(self.all_targets[self.all_targets["GEOID"]==id].values[-1:,3]) # DELTA_CASE_JH
...
def train_test_splits(current_date, init_df, temporal_lag=TEMPORAL_LAG, forecast_horizon=FORECAST_HORIZON, print_info=False):
    # create data object from csv
    data = covidData(current_date=current_date,
                     init_df=init_df,
    ...
    if print_info:
        print("Number of training instances:", n_train, ", Number of test instances:", n_test)
