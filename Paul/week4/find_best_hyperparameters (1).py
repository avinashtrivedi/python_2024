...
from check_lstm_model import ensure_model_is_correct
...
class QuantileLoss(nn.Module):
    ...
    def forward(self, outputs, targets):
        ...
        """
        # your code goes here
        # make sure your loss returns a single number (of datatype tensor) and not an array tensor
        pass
...
def eval_model(lstm_model, test_loader, y_delta_cases, test_info):
    ...
    with torch.no_grad():
        ...
        for quantile in QUANTILES:
            ...
            results_df["q_"+quantile+"_pred_transform"] = np.exp(results_df["q_"+quantile+"_pred"])-1
            results_df["q_"+quantile+"_pred_cases"] =  results_df["q_"+quantile+"_pred_transform"]*results_df["POPULATION"]/10000
            
        results_df["y_lbl_transform"] = np.exp(results_df["y_label"])-1
        results_df["y_label_transformed"] = results_df["y_lbl_transform"]*results_df["POPULATION"]/10000
        
        # error = absolute difference between the predicted delta cases at the 50th quantile and the true delta cases
        results_df['y_q500_err'] = abs(results_df['y_delta_cases'] - results_df['q_500_pred_cases'])
        results_df.drop(cols, axis=1, inplace=True)
        
    return results_df

def eval_results(burnin_weeks, num_of_biweekly_intervals, state, county, temporal_lag, forecast_horizon):
    ...
    """
    # once you have coded the LSTM_Model and QuantileLoss classes above, remove the statement below
    return None, None, None

    # the next line checks if the model has been defined correctly for the auto-grader
    ensure_model_is_correct(LSTM_Model())

    initial_date = date(int(dc.INITIAL_DATE.split("-")[0]),
    ...
    for i in range(num_of_biweekly_intervals):
        ...
        X_train_ts, y_train, X_test_ts, y_test, y_delta_cases, test_info = \
            dc.train_test_splits(forecast_date.strftime('%Y-%m-%d'),
                                 wdf,
        ...
        if i%5 == 0 or i == num_of_biweekly_intervals-1:
            print(f"Training interval {i+1} for {forecast_date} with {X_train_ts.shape[0]} train samples (state)")
        ...
        if county is not None:
            wdf = df[(df['STATE'] == state) & (df['NAME'] == county)] # filter the records for the county in the state
            if i == 0:
            ...
            if i%5 == 0 or i == num_of_biweekly_intervals-1:
                print(f"Training interval {i+1} for {forecast_date} with {X_train_ts.shape[0]} train samples (county)")
    ...
    for i in range(num_of_biweekly_intervals):
        ...
        if county is not None:
            ...
            err_state.append(df_state['y_q500_err'].mean()) # take the mean error for all counties in the state
    
    if county is None:
        err_results_df = pd.DataFrame({'error(State)': err_state,
